...
#include "sensor_io_i2c_sensesp.h"      // Has functions for I2C reading/writing
...
int8_t FXOS8700_ReadAccData(struct PhysicalSensor *sensor, SensorFusionGlobals *sfg)
{
    ...
    //Steady state when fusing at 40 Hz is 5 packets per cycle to read (accel updates at 200 Hz).
...
#define MAX_FIFO_PACKETS_PER_READ 11        
    ...
    while( (fifo_packet_count > 0)  && (status==SENSOR_ERROR_NONE)) {
        if( MAX_FIFO_PACKETS_PER_READ < fifo_packet_count ) {
            FXOS8700_DATA_READ[0].numBytes = MAX_FIFO_PACKETS_PER_READ * 6;
        ...
        status = Sensor_I2C_Read(sensor->bus_driver, &sensor->deviceInfo,
                                    sensor->addr, FXOS8700_DATA_READ,
                                    I2C_Buffer);
        if (status==SENSOR_ERROR_NONE) {
            ...
            }//end transfer one sample to accelerometer data struct
        }//end transfer all bytes read in single burst read
