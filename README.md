...
It is configured to work with these *Adafruit breakout boards*:
- #3643 with **NXP FXOS8700** magnetometer/accelerometer and **FXAS21002** gyroscope
sensor ICs
- #4517 with **STM LIS3MDL** magnetometer and **LSM6DSOX** gyroscope+accelerometer
but can be modified to work with other sensors having an I2C
...
interfaces. For a more complex example, see the adjunct library 
...
My motivation for porting NXP's library is to create an orientation sensor for marine use, using off-the-shelf hardware and the [Signal K / SensESP project](https://github.com/SignalK/SensESP) to provide orientation data: **magnetic heading, roll, and pitch on a vessel**. 

I targeted this project for the Espressif ESP32/ESP8266 microcontrollers, and the NXP & STM sensors listed earlier.  


## Sensors
The present software works with two different 9DoF sensor combinations:
- NXP's **FXOS8700** magnetometer + accelerometer and **FXAS21002** gyroscope. These were available mounted together on the **Adafruit #3463 breakout** board, which is not in production at this time. These particular ICs are available on other breakout boards found on AliExpress (as of March 2026)
- STMicroelectronics' **LIS3MDL** magnetometer and **LSM6DSOX** gyroscope + accelerometer.  These two ICs are available ($20 as of March 2026) on the **Adafruit #4517 breakout** board.
With some work, this library can be ported to other sensor ICs. Note that only the I2C interface has been implemented and tested; a SPI interface is possible with additional work and testing.

## Processor
The present software is written for the ESP32 and ESP8266 processors. With some rewriting of the I2C and timing routines, the code of this library can be ported to other processors.

If wanting to send the orientation data via Signal K, then you *should choose the ESP32 processor*, as the **SensESP/Signal K** library is not intended for the ESP8266. 
...
Now that you know your development environment works, follow either of these two methods to bring in the library files as a new project:
...
- in PlatformIO, select *File->Open Folder* and browse to your local copy of this library. This will create a new Project, based on the local library copy.
- in PlatformIO's File Explorer, copy `/examples/example_main.cc` into your new project's `/src` folder. It contains the `setup()` and `loop()` functions, and you can rename it `main.cc` or `main.cpp` if desired. 

### Method 2 (doesn't require manual cloning into a local repository)
- copy this project's `/examples/example_main.cc` into your new PlatformIO project's `/src` folder - replacing your existing `main.cpp`. 
- copy this project's `platformio.ini`into your project's root directory, replacing your existing `platformio.ini`. Alternatively, use it to modify the relevant sections in your own project's `platformio.ini`).
- Locate the section `lib_deps =` in the `platformio.ini` file and add the line https://github.com/BjarneBitscrambler/OrientationSensorFusion-ESP.git. 

Method 2 results in this library's code being automatically imported into PlatformIO's `.pio` folder.
...
- edit the `#defines` at the top of your `example_main.cc` to reflect your particular hardware. The GPIO pins connecting your processor to your sensor ICs (the I2C connection) will likely be different, and you may also need to change the I2C addresses that the ICs are configured for.
- compile and download to your ESP board.
...
You can use the GPIO output that toggles each time through the data collection and sending loop to confirm whether your ESP is collecting and transmitting data regularly. Using the default software, the output should toggle every 25 ms (i.e. a 20 Hz square wave). See `example_main.cc` for details.

### Customizing and Modifying

The file `build.h` contains defines for various functionality, such as whether the software outputs its data via hardware serial UART or WiFi TCP connections, or both (default). Edit this file as desired, but note that not all combinations of features may be valid or been tested.
...
Files that would be expected to change when using different hardware are the `hal_*.*` files, `board.h`, and `build.h`.  As well, new sensor IC driver files may be needed, patterned on the existing `sensors/driver_fxos8700.*`, `sensors/driver_fxas21002.*`, `sensors/driver_lis3mdl.*`, and `sensors/driver_lsm6dsox.*` files. Finally, `fusion/calibration_storage.*` may need changing depending on how non-volatile memory functions on the different hardware.

If you want to **change how the fusion algorithm operates**, have a look at `build.h`, `fusion/control*.*`, and `fusion/status.*`. Quite a lot of parameters are selected via pre-processor `#define` statements; check the comments for suggestions on how to achieve your goals. 
